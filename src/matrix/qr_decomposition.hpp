...
{
private:
    Matrix<T> create_householder_matrix(const Matrix<T> &vector, const Matrix<T> &basis)
    {
        ...
        Matrix<T> unit_vector = vector.subtract(
                                          basis.el_multiply(alpha))
                                    .el_divide(vector.norm(2));
    ...
    void operator()(const Matrix<T> &matrix)
    {
        ...
        for (int i = 0; i < n_iter; ++i)
        {
            ...
            Matrix<T> q_i = create_householder_matrix(
                matrix.extract_submatrix(i, i).extract_column(0),
