 ...
 */
template <Number T> class Arithmetic
{
  public:
                                           ...
                                           std::function<T(T, T)> &binary_op) const
    {
        ...
        std::transform(left.begin(), left.end(), right.begin(), std::back_inserter(out),
                       [&binary_op](const std::vector<T> &x1, const std::vector<T> &x2) {
                           std::vector<T> temp;
                           std::transform(x1.begin(), x1.end(), x2.begin(),
                                          std::back_inserter(temp), binary_op);
                           return temp;
                       });
        return out;
    }

    std::vector<std::vector<T>> operator()(const std::vector<std::vector<T>> &data, T scalar,
                                           std::function<T(T, T)> binary_op) const
    {
        std::vector<std::vector<T>> out;
